...
var ponyclicker = (function () {
    ...
    function ParseGame(src) {
        ...
        switch (g.version) {
            ...
            case 4:
                ...
                g.version = 5;
            case 5:
                g.settings.wheelDisplay = 1 /* expanded */;
                g.version = 6;
            case 6:
                Game = g;
                break;
    ...
    var StoreItem = (function () {
        function StoreItem(sItem, id) {
            if (sItem.cost !== undefined)
                this.cost = sItem.cost;
            if (sItem.formula !== undefined)
                this.formula = sItem.formula;
            this.name = sItem.name;
            this.plural = sItem.plural;
            this.desc = sItem.desc;
            this.img = sItem.img;
            this.fn_SPS = sItem.fn_SPS;
            if (sItem.initcost !== undefined)
                this.initcost = sItem.initcost;
            if (sItem.costcurve !== undefined)
                this.costcurve = sItem.costcurve;
        }
        StoreItem.prototype.cost = function (n) {
            return this.initcost * Math.pow(this.costcurve, n) * (1 + poisson(n + 3, 14) * 5);
        };
        return StoreItem;
    }());
    function generateStore(items) {
        function inv_cost(i, cost) { return Math.floor(Math.log(cost / items[i].initcost) / Math.log(items[i].costcurve)); }
        ...
        function initSPS(item, store) { return item.fn_SPS(store); }
        function initcost(i, store) { return initSPS(items[i], store) * get_rratio(i); }
    ...
    var Store = generateStore([
        {
            ...
            img: function (n) { return 'ponies/' + PonyList[(n >= Game.ponyList.length ? 0 : Game.ponyList[n])] + '.svg'; },
            fn_SPS: function (store) { return 0; }
        },
        {
            cost: fn_cost1,
            costcurve: fcurve,
            ...
            img: function (n) { return 'store/hoofbump.svg'; },
            fn_SPS: function (store) { return 1.0; }
        },
        {
            name: "Recital",
            plural: "recitals",
            ...
            img: function (n) { return 'store/cello.svg'; },
            fn_SPS: function (store) { return store[0]; }
        },
        {
            name: "Party",
            plural: "parties",
            ...
            img: function (n) { return 'store/balloon.svg'; },
            fn_SPS: function (store) { return store[1]; }
        },
        {
            name: "Parade",
            plural: "parades",
            ...
            img: function (n) { return 'store/trixie_wagon.svg'; },
            fn_SPS: function (store) { return store[0] + store[1]; }
        },
        {
            ...
            img: function (n) { return 'store/octavia_cutiemark.svg'; },
            fn_SPS: function (store) { return store[0] + store[1] + store[2] + store[3] + store[4]; }
        },
        {
            name: "Festival",
            plural: "festivals",
            ...
            img: function (n) { return 'store/stage.png'; },
            fn_SPS: function (store) { return store[0] * store[1]; }
        },
        {
            name: "Rave",
            ...
            img: function (n) { return 'store/turntable.png'; },
            fn_SPS: function (store) { return store[0] * store[1] * store[5]; }
        },
        {
            ...
            img: function (n) { return 'store/redhat.svg'; },
            fn_SPS: function (store) { return store[0] * store[1] * (store[3] + store[4]); }
        },
        {
            name: "Coronation",
            plural: "coronations",
            ...
            img: function (n) { return 'store/twilicorn_crown.svg'; },
            fn_SPS: function (store) { return store[0] * store[1] * store[5] * store[7]; }
        },
        {
            ...
            img: function (n) { return 'store/calendar.svg'; },
            fn_SPS: function (store) { return store[0] * store[1] * store[4] * store[6] * store[9]; }
        },
        {
            ...
            img: function (n) { return 'store/' + ElementList[n % ElementList.length] + '.svg'; },
            fn_SPS: function (store) { return store[0] * store[1] * Math.exp(Math.pow(countb(store) * store[1], 1 / 4)); }
        } //P*F*exp((F*B)^1/4)
    ]);
    function factorial(n) { var r = n; while (--n > 1) {
        r = r * n;
    } return r; }
    function factorial_limit(n, k) { var r = n; while (--n > k) {
        r = r * n;
    } return r; }
    function max_binomial(n) { var n2 = Math.floor(n / 2); var r = n; while (--n > n2) {
        r = r * n;
    } return Math.floor(r / factorial(n2)); }
    ...
    function countb(store) { var r = 0; for (var i = 2; i < store.length; ++i)
        r += store[i]; return r; }
    /*
    ...
    */
    ...
    function ResetGame() {
        ...
        if (Game.totalsmiles > 1e9)
            EarnAchievement(212);
        if (Game.totalsmiles > 1e12)
            EarnAchievement(213);
        if (Game.totalsmiles > 1e15)
            EarnAchievement(214);
        if (Game.totalsmiles > 1e18)
            EarnAchievement(215);
        if (Game.totalsmiles > 1e21)
            EarnAchievement(216);
        if (Game.totalsmiles > 1e24)
            EarnAchievement(217);
        if (Game.totalsmiles > 1e27)
            EarnAchievement(218);
        if (Game.totalsmiles > 1e30)
            EarnAchievement(219);
    ...
    function LoadGame() {
        if (localStorage.getItem('game') !== null) {
            ParseGame(localStorage.getItem('game'));
        }
        ApplySettings();
    }
    ...
    function GetSettings() {
        ...
        if ($('#numdisplay0').prop('checked'))
            Game.settings.numDisplay = 0 /* names */;
        if ($('#numdisplay1').prop('checked'))
            Game.settings.numDisplay = 1 /* rawNumbers */;
        if ($('#numdisplay2').prop('checked'))
            Game.settings.numDisplay = 2 /* scientificNotation */;
        if ($('#wheeldisplay0').prop('checked'))
            Game.settings.wheelDisplay = 0 /* entire */;
        if ($('#wheeldisplay1').prop('checked'))
            Game.settings.wheelDisplay = 1 /* expanded */;
        if ($('#wheeldisplay2').prop('checked'))
            Game.settings.wheelDisplay = 2 /* pony */;
        UpdateSPS();
        OrganizePonies();
    }
    function ShowLegacyStats() {
        var pclicks = $('#statlegacyclicks')[0], psmiles = $('#statlegacysmiles')[0], presets = $('#statresets')[0], pcupcakes = $('#statcupcakes')[0];
    ...
    function GetNews() {
        ...
        if (Game.totalsmiles < 30) {
            news.push("Ponyville down in the dumps, reports anonymous bystander.", "Ponyville reports distinct lack of smiles.", "Smile forecast for today: Depression with a touch of despair.", "Ponyville in desperate need of excitement.", "Antidepressants selling out all across Ponyville.", "Mayor Mare asked why everyone is feeling so down, reports she can't be bothered to figure it out.");
        }
        else if (Game.totalsmiles < 1000) {
            news.push("Ponyville citizens wonder what this strange new sensation is! Scientists call it 'smiling', conspiracy theorists denounce it as a ploy to take over the world.", "Morose ponies increasingly out of place in chipper Ponyville!", "Small foals spotted playing in the streets! Parents unsure if so called 'fun' should be allowed.", "Chef proposes selling 'baked goods' instead of 'baked okays'. Customers wary of new marketing scam.");
        }
        else if (Game.totalsmiles < 1000000) {
            news.push("Ponies greeting each other with a smile! Cranky old ponies decry new development!", "Iron Will now teaching assertiveness instead of methods of coping with depression!", "Several young colts spotted hoofbumping! Parents concerned over new trend!", "Therapist upset about recent mood upswing, says it's bad for business.");
        }
        else if (Game.totalsmiles < 1000000000) {
            news.push("Songs now spontaneously erupting across Ponyville!", "Ponyville voted happiest town in equestria!", "Ponies from all over Equestria come to visit Ponyville!");
        }
        else if (Game.totalsmiles < 1000000000000) {
            news.push("Princess Twilight found overdosed on friendship, taken to rehab center!", "Citizens of Ponyville so happy they invent new word for it! Debates about how to spell it immediately turn into murderous riots!");
        }
        ...
        if (Game.totalsmiles > 10000) {
            news.push('Twilight found shipping Rainbow Dash with everything in the universe! Riots erupt all across Equestria!', 'Lyra and BonBon revealed as "just friends"! Ponies everywhere faint in shock! Octavia and Vinyl Scratch refuse to comment.', "Celestia's insatiable desire for cake causes caketastrophe in the Royal Kitchen! A memorial service for the lost chocolate chips to be held on Monday.", "Pink menace at Sugarcube corner goes batty, takes 15 muffins hostage!", "Citizens of Ponyville vote to create a public library instead of relying on a private collection organized by a crazed purple mare!", "Small colt finds lost Apple barn floating in space. Rainbow Dash claims she has no idea how it got up there.", "Rarity joins environmentalists, declares she will no longer go to the spa. Ponyville's spa immediately goes bankrupt.", "Following an incident involving mislabeled sodium and an exploding toilet, Celestia orders Sweetie Bot to register herself as a lethal weapon.", "Rainbow Dash reportedly investing in the Cloud. Pegasi everywhere confused by what this means.", "Princess Twilight discovers that ponies are actually tiny nuclear reactors! \"That explains why I never need to go to the bathroom,\" says Rainbow Dash.", "Pony pony Pony pony pony pony Pony pony!", "Princess Twilight Sparkle dating a peach! The peach has no comment on the matter.", 'Doctor Whooves bumps into himself. Ponyville citizens worried that there will be "No more."', 'Big Mac and Smarty Pants have deep philosophical conversations. When asked what he sounds like, Smarty Pants refused to comment.', 'Apple Bloom found in shock on the edge of the Everfree Forest; says she visited a village of ponies with no Cutie Marks.', 'Applejack finds golden apple. Looks away awkwardly and quickly changes the subject when asked how she found it.', 'Pinkie Pie found running around Ponyville, proclaiming "Our lives aren\'t reality and that we\'re just a bunch of animated characters on a TV show meant for the entertainment of others!"');
            if (Game.muffins > 10) {
                news.push('When asked how she saved the bakery from certain disaster, Derpy Hooves claims there was "Muffin to it!"', "Try new smile-powered Muffins today!", 'Mayor held hostage by crazed Doctor, who demands that a muffin factory be built "for the sake of all ponykind!"', 'Derpy and Troubleshoes get married! Ponyville does not survive the wedding.', 'Scientists investigate whether excessive muffin consumption can lead to long, overbearing plots.', 'Rarity to design new fashion line for plus sized mares, claiming “Muffin tops are in fashion this season”.', "Rainbow Dash to host new midnight release party for AK Yearling’s latest novel, ‘Daring Do and the Muffin Man of Azkaban’.", "Fluttershy cancels bi-weekly critter picnic in exchange for new Muffin Social. Critters could not be reached for comment.", "Applejack introduces apple-spice muffins, in attempt to reignite declining produce sales.", "Twilight Sparkle under house arrest for magical mammal manipulation, for trying to cross a mouse with a muffin.", "Pinkie Pie launches new re-branding of cupcakes as ‘dessert muffins’, meets with mixed results.");
            }
            if (Game.muffins > 100) {
                news.push('Derpy crashes into giant muffin. Irony is not amused.', "Discord caught eating 40 muffins in one sitting, that’s as many as four 10’s, and that’s terrible.", "Princess Celestia has eaten at least 37 muffins this year, when reached for comment, Princess Luna responded with ‘In a row?’.", "Princess Cadence Announces that all muffins produced in the Crystal Empire now come with ‘Free Shipping’.  Her Highness then winked suggestively.", "As muffin craze sweeps Equestria, Sapphire Shores to star in new musical, ‘My Little Muffin, Baked-goods are Magic’.", "Diamond Tiara insists that her father has a bigger muffin collection than you, no matter how improbable that sounds.", "New Poll from the Foal Free Press reveals that no food makes a young filly smile as much as a muffin.", 'Sugarcube Corner holds muffin bake sale. Ponyville mourns the loss of many ponies during the ensuing muffin frenzy.', "Chrysalis returns! Says she's just buying muffins.");
            }
            if (Game.muffins > 200) {
    ...
    var newsnum = 0, newswait = 15000, lastNews;
    function UpdateNews() {
        var n2 = (newsnum + 1) % 2;
        $news.children()
    ...
    function NumCommas(x) {
        if (x < 1e20) {
            ...
            if (!len)
                len = 3;
            var r_1 = n.substring(0, len);
            for (var i = len; i < n.length; i += 3) {
                r_1 += ',' + n.substring(i, i + 3);
            }
            return r_1;
        }
    ...
    function GetNumberName(d) {
        ...
        if (n <= number_names.length)
            return number_names[n - 1];
        if (n > 999)
            return "Infinity"; // this is actually impossible to reach because doubles don't go that high but I left it in here anyway.
        var d1 = (n % 10);
        ...
        switch (d1) {
            case 3:
                if (NumberFlag(d2, d3, 3))
                    n1 += "s";
                break;
            case 6:
                if (NumberFlag(d2, d3, 1))
                    n1 += "s";
                else if (NumberFlag(d2, d3, 2))
                    n1 += "x";
                break;
            case 7: // if flags has n, add n, if m, add m
            case 9:
                if (NumberFlag(d2, d3, 4))
                    n1 += "n";
                else if (NumberFlag(d2, d3, 8))
                    n1 += "m";
                break;
        }
        var s = n1 + n2 + n3;
    ...
    function PrintTime(time) {
        ...
        t[0] = (time - t[1]) / 365;
        if (t[0] > 100)
            return "Centuries"; // more than 100 years
        for (var i = 3; i <= 4; i++)
            if (t[i] < 10)
                t[i] = "0" + t[i];
        var output = (t[2] > 0 ? (t[2] + ":") : "") + t[3] + ":" + t[4];
        if (t[1])
            output = t[1] + " day" + (t[1] !== 1 ? 's' : '') + " and " + output;
        if (t[0])
            output = t[0] + " year" + (t[0] !== 1 ? 's' : '') + ", " + output;
        return output;
    }
    ...
    function gen_finalupgrade(m) { return function (sps, store) { var b = (store[0] + store[1] + CountBuildings(store)) * m; for (var i = 0; i < sps.mStore.length; ++i)
        sps.mStore[i] += b; return sps; }; }
    ...
    function genprecond(id) {
        var _this = this;
        return function () { return (Game.upgradeHash[id] !== undefined) && (_this.cost < (Game.totalsmiles * 1.1)); };
    }
    var upgrade = (function () {
        function upgrade(upg) {
            ...
            this.desc = upg.desc;
            this.fn = upg.fn;
            if (upg.cond !== undefined)
                this.cond = upg.cond;
            if (upg.flavor !== undefined)
                this.flavor = upg.flavor;
        }
        upgrade.prototype.cond = function () {
    ...
    var achievementList = {
        ...
        '206': { name: "No Booping Allowed", desc: "Get <b>" + PrettyNumStatic(1000000000000, false, 0) + "</b> smiles with only 35 pony boops.", muffins: 1, cond: function () { return Game.clicks <= 35 && Game.totalsmiles >= 1e12; } },
    ...
    var achievements_clicks = genAchievements(["That tickles!", "Tickle War", "Tickle War II: The Retickling", "This Can't Be Healthy", "Carpal Tunnel Syndrome", "Wrist In Pieces", "It's Over Nine Thousand!"], [10, 100, 500, 1000, 2500, 5000, 9001], function (n) { return "Boop a pony <b>" + PrettyNumStatic(n, false, 0) + "</b> times."; }, function (n) { return function () { return Game.clicks >= n; }; });
    achievements_clicks.push(2);
    var achievements_smiles = genAchievements(["Joy", "Glee", "Bliss", "Nirvana", "Ecstasy", "Pursuit of Happyness", "You Can Stop Now", "This Is Ridiculous", "Go Read A Book", "How?!"], [100, 10000, 1000000, 100000000, 10000000000, 1000000000000, 100000000000000, 10000000000000000, 1000000000000000000, 100000000000000000000], function (n) { return "Get <b>" + PrettyNumStatic(n, false, 0) + "</b> smiles."; }, function (n) { return function () { return Game.totalsmiles >= n; }; });
    ...
    function genShopAchievements(item, names) {
        return genAchievements(names, [1, 50, 100, 150, 200], function (n) { return "Buy <b>" + Pluralize2(n, "</b> " + Store[item].name.toLowerCase(), "</b> " + Store[item].plural.toLowerCase(), false, 0) + "."; }, genShopCond(item));
    }
    ...
    achievements_shop = achievements_shop.concat(genAchievements(["Loyalty", "By Your Powers Combined", "Honesty", "Laughter", "Generosity", "Kindness"], [1, 6, 20, 40, 80, 160], function (n) { return "Buy <b>" + Pluralize2(n, "</b> " + Store[11].name.toLowerCase(), "</b> " + Store[11].plural.toLowerCase(), false, 0) + "."; }, genShopCond(11)));
    achievements_shop = achievements_shop.concat(genAchievements(["Best Buds", "Camaraderie Crusaders", "Inner Circle", "Friend-Of-The-Month Club", "No Pony Left Behind", "Reference Chart Not Optional"], [2, 3, 6, 12, 18, 24], function (n) { return 'Have at least <b>' + n + ' ponies</b> and a completed friendship graph.'; }, function (n) { return function () { return (Game.store[0] >= n) && (Game.store[1] >= triangular(n)); }; }));
    ...
    function ResizeCanvas() {
        ...
        if (Game.settings.wheelDisplay == 0 /* entire */)
            OrganizePonies();
    }
    ...
    function Buy(id) {
        var n = Game.shiftDown ? 10 : 1, numPurchase = 0, totalCost = 0;
        for (var i = 0; i < n; ++i) {
    ...
    function Sell(id) {
        if (!id)
            return; // you can't sell ponies you heartless monster
        var n = Game.shiftDown ? 10 : 1, numSell = 0, totalCost = 0;
        for (var i = 0; i < n; ++i) {
            if (Game.store[id] > 0) {
                Game.store[id] -= 1;
    ...
    function updateUpgradesAchievements() {
        ...
        var makeAchievement = function (iterator, prop, upgrade) {
            ...
            $(document.createElement('div'))
                ...
                .appendTo($ach);
            if (!upgrade)
                $(document.createElement('div')).addClass('muffin').html('+' + achievementList[prop].muffins).appendTo($ach);
            if (Game.achievements[prop] == null && !upgrade)
                $ach.addClass('hidden');
            return $ach;
        };
        $achievements.empty();
    ...
    for (var i = 0; i < Store.length; ++i) {
        updatestore_nstore.push(0);
    }
    function UpdateStore() {
        var minPayPerSmile = Number.MAX_VALUE, minItem = -1;
        if (Game.settings.showHighlight) {
            // Find the most efficient building
            for (var i = 0; i < Store.length; ++i) {
                for (var j = 0; j < Store.length; ++j)
                    updatestore_nstore[j] = 0 + Game.store[j];
                updatestore_nstore[i] += 1;
                var nSPS = CalcSPS(updatestore_nstore, Game.upgrades, Game.cupcakes, false), payPerSmile = Store[i].cost(Game.store[i]) / (nSPS - Game.SPS);
        ...
        for (var i = 0; i < Store.length; ++i) {
            var item = Store[i], count = Game.store[i], cost = item.cost(count), $buyN = $("#buy" + i);
            ...
            if (minItem == i)
                $buyN[0].style.cssText = 'color:#aaffaa !important;'; //jQuery uses a lot of memory for some reason???
            else
                $buyN[0].style.cssText = 'color:#fff;';
            $("#cost" + i).html(PrettyNum(cost));
            $("#count" + i).html(count.toString());
            var elem = $("#icon" + i)[0];
            var nimg = "url('" + Store[i].img(Game.store[i]) + "')";
            if (nimg != elem.style.backgroundImage)
                elem.style.backgroundImage = nimg; // helps alleviate memory leaks
        }
    ...
    function displayTime(milliseconds) {
        var seconds = Math.floor(milliseconds / 1000) % 60, minutes = Math.floor(milliseconds / 60000) % 60, pad = function (n) { return n < 10 ? '0' + n : n; };
        return Math.floor(milliseconds / 3600000).toFixed(0) + ':' + pad(minutes.toFixed(0)) + ':' + pad(seconds.toFixed(0));
    }
    function drawImage($img, x, y, r) {
        var img = $img[0], w = $img.width(), h = $img.height();
        if ($img.doneLoading) {
            if (r != 0) {
    ...
    function GameTick() {
        ...
        var hasFocus = !Game.settings.optimizeFocus || document.hasFocus(), framelength = (hasFocus ? 33 : 500); // 33 is 30 FPS
        if (Game.delta > framelength) {
    ...
    function UpdateOverlay(item, y, mobile) {
        ...
        if (!skip) {
            ...
            if (item < 0)
                return $overlay.hide();
            var x = Store[item], xcount = Game.store[item], xcost = x.cost(xcount), $title = $(document.createElement('div'))
                .addClass('title')
                .append('<p>' + x.name + '</p><span>' + smilethumb + PrettyNum(xcost) + '</span>');
            if (xcount > 0)
                $title.append('<div>[' + PrettyNum(xcount) + ' owned]</div>');
            $overlay.empty().append($title, '<hr><p>' + x.desc + '</p>'); //<ul>
            var $ul = $(document.createElement('ul'));
            if (x.formula)
                $ul.append('<li class="formula">' + x.formula + '</li>');
            if (x.SPS_cache > 0 || item == 1)
                $ul.append('<li>Each ' + x.name.toLowerCase() + ' generates <b>' + Pluralize(x.SPS_cache, ' smile') + '</b> per second</li>');
            if (xcount > 0 && x.SPS_cache > 0)
                $ul.append('<li><b>' + PrettyNum(xcount) + '</b> ' + x.plural.toLowerCase() + ' generating <b>' + Pluralize(xcount * x.SPS_cache, ' smile') + '</b> per second</li>');
            ...
            for (var i = 0; i < Store.length; ++i) {
                updateoverlay_nstore[i] = 0 + Game.store[i];
            } //ensure javascript isn't passing references around for some insane reason
            updateoverlay_nstore[item] += 1;
            var nSPS = CalcSPS(updateoverlay_nstore, Game.upgrades, Game.cupcakes, false), sps_increase = nSPS - Game.SPS, payPerSmile = xcost / (nSPS - Game.SPS), increaseText = sps_increase > 0 ? 'will increase your SPS by <b>' + (sps_increase > lowerbound ? PrettyNum(sps_increase) : 'almost nothing') + '</b>' : "<b>won't</b> increase your SPS", payPerSmileText = isFinite(payPerSmile) ? '<i>You pay <b>' + (sps_increase > lowerbound ? Pluralize(payPerSmile, ' smile') : 'way too many smiles') + '</b> per +1 SPS</i>' : '';
            $ul.append('<li>Buying one ' + x.name.toLowerCase() + ' ' + increaseText + payPerSmileText + '</li>');
            if (xcost > Game.smiles && Game.SPS > 0)
                $ul.append('<li>This can be purchased <span id="overlaytime" data-item="' + item + '">' + CalcTimeItem(item) + '</span></li>');
            // Display buy/sell information
            var helpStr = '<li><kbd>Shift + Click</kbd> to buy 10';
            if (xcount > 0 && item > 0)
                helpStr += ', <kbd>Right click</kbd> to sell 1'; // you can't sell ponies
    ...
    function UpdateUpgradeOverlay(item, x, y) {
        var skip = false;
        if (item != null && item >= curUpgradeList.length)
            item = -1;
        if (y != null && item >= 0) {
            if (y > ($storeupgrades.get(0).offsetHeight + $storeupgrades.offset().top))
                item = -1;
        }
        if (item == null)
            ...
            skip = true;
        if (!skip) {
            if (item >= curUpgradeList.length)
                item = -1; // This edge case happens when you buy all the upgrades
            $upgradeoverlay.attr('data-item', item);
            if (item < 0)
                return $upgradeoverlay.hide();
    ...
    function OrganizePonies() {
        var n = Game.store[0], radd = n * 30, r = 140 + radd, a = (2 * Math.PI) / n, th = (n - 2) * 0.08;
        if (Game.settings.wheelDisplay == 0 /* entire */)
            r = pb.offsetHeight * 0.25;
        if (n == 1)
            r = 0;
        document.getElementById('ponyspin').style.top = (Game.settings.wheelDisplay == 0 /* entire */) ? (-r * 0.5) + 'px' : '120px';
        var edge = GetEdgeLength(r, n);
        $ponywrapper.empty();
        for (var i = 0; i < n; ++i) {
            var pone = ((i < Game.ponyList.length) ? Game.ponyList[i] : 0), $ponyDiv = $(document.createElement('div'))
                .attr('id', 'pony' + i)
                .addClass('pony')
                .css({
    ...
    var setMouseMove = function (event) {
        var root = $('#buy0')[0], wrapper = $('#storewrapper')[0], item = -1, actualTop = root.offsetTop - wrapper.scrollTop + wrapper.offsetTop, mobile = $doc.width() < 600;
    ...
    if (typeof disable_ponyclicker !== 'undefined')
        return public_members;
    //
    // -------------------------------- Begin Game Initialization --------------------------------
    //
    var $doc = $(document), $w = $(window), $loadscreen = $('#loadscreen'), $EnableE = $('#EnableEffects'), $EnableF = $('#EnableFocus'), $EnableW = $('#EnableWarn'), $EnableH = $('#EnableHighlight'), $menu = $('#menu'), $score = $("#scorenum"), $store = $("#store"), $SPS = $("#SPS"), $stats = $menu.children('.stats'), $stat_cursmiles = $stats.find('.cursmiles'), $stat_totalsmiles = $stats.find('.totalsmiles'), $stat_SPS = $stats.find('.SPS'), $stat_clicks = $stats.find('.clicks'), $stat_SPC = $stats.find('.SPC'), $stat_buildings = $stats.find('.buildings'), $stat_time = $stats.find('.time'), $stat_muffins = $stats.find('.muffins'), $stat_legacysmiles = $stats.find('.legacysmiles'), $stat_resets = $stats.find('.resets'), $stat_legacyclicks = $stats.find('.legacyclicks'), $stat_cupcakes = $stats.find('.cupcakes'), $stat_clonespopped = $stats.find('.clonespopped'), $achievements_owned = $('#achievements_owned'), $achievements_total = $('#achievements_total'), $upgrades_owned = $('#upgrades_owned'), $upgrades_total = $('#upgrades_total'), $ponywrapper = $('#ponywrapper'), $achievements = $('#achievements'), smilethumb = '<img src="pinkiehappy.png" alt="Smiles: " title="Smiles" />', canvas = document.getElementById('canvas'), $canvas = $(canvas), ctx = canvas.getContext("2d"), canvaslines = document.getElementById('canvaslines'), $canvaslines = $(canvaslines), ctxlines = canvaslines.getContext("2d"), $img_rays = CacheSVG('rays.svg'), // the onload check is done for firefox, which gets overeager
    $img_ground = CacheSVG('ground.svg'), $overlay = $('#overlay'), $upgradeoverlay = $('#upgradeoverlay'), $storeupgrades = $('#storeupgrades'), $upgradelist = $('#upgradelist'), $ponycost = $('#ponycost'), $menubtn = $('#menubutton'), $notices = $('#notices'), $storewrapper = $('#storewrapper'), $board = $('#ponyboard'), $news = $board.children('.newsactive'), pbg = $('#ponybg')[0], $mousetexts = $('#mousetexts'), $pinkieclones = $('#pinkieclones'), $statclonespopped = $('#statclonespopped'), pb = $board[0];
    var Game = new CreateGame(), curMouseX = 0, curMouseY = 0, pinkie_freelist = [];
    ...
    for (var i = 0; i < Store.length; ++i) {
        var $item = $(document.createElement('li'))
            ...
            .attr('id', 'icon' + i)
            .addClass('icon')
            .css({ backgroundImage: "url('" + Store[i].img(Game.store[i]) + "')" })), $costSpan = $(document.createElement('span'))
            .addClass('cost')
            .append(smilethumb, $(document.createElement('span'))
            .attr('id', 'cost' + i)
            .html('0')), $countSpan = $(document.createElement('span'))
            .attr('id', 'count' + i)
            .addClass('count')
            .text(0);
        $item.append($costSpan);
        if (i == 1)
            $item.append($ponycost = $(document.createElement('span')).attr('id', 'ponycost').text('(NEEDS 2 PONIES)'));
        $item.append($countSpan);
        appendStoreClick($item, i);
        $store.append($item);
    }
    var $showmenu = $('#showmenu').on('click', function () { ShowMenu(true); }), $hidemenu = $('#hidemenu').on('click', function () { ShowMenu(false); }), $exportWindow = $('#exportwindow'), $credits = $('#credits');
    ...
    $('#pagealert').on('click', function () {
        SaveGame();
        Game.settings.closingWarn = false;
    ...
    $w.on('load', function () {
        $doc
            ...
            .on('mouseup', function (event) { if (event.which === 1)
            fnmouseup(event); })
            .on('touchend touchcancel', function (event) { fnmouseup(event); })
            .on('mousedown', function (event) { if (event.which === 1) {
            vlastangle = vangle;
        } })
